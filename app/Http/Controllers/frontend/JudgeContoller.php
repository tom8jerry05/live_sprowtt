...
class JudgeContoller extends Controller
{
   /* public function index()
    {
    	$countries  = Country::select('id','name')->get();
        $projects = Tombstone::select('id','name','image_link')->get();
    	return view('frontend.judgeform')->with(compact('countries'))->with(compact('projects'));
    }*/
    ...
    public function index($id){
        ...
        return view('frontend.judgeform')->with(compact('countries'))->with(compact('projects'))->with(compact('tombstone'));
    }
    ...
    public function create(Request $request)
    {
    	...
    	$validator = Validator::make($request->all(),[
            ...
            'city'        => 'required',
            'projectId'   => 'required',
    		'email'       => 'required|email|unique:judge,email|unique:users,email',
    		'phonenumber' => 'required|numeric|unique:judge,phonenumber',
    	]);

    	if($validator->fails()){
            $errors = $validator->messages();
            return redirect()->route('Judge',$request->input('projectId'))->withInput()->with('errors',$errors);
    	}else{
            ...
            $judge->project_id  = $request->input('projectId');
            ...
            if($judge->save()){
                ...
                if($user->save()){
                    ...
                    $data = array(
                        ...
                        'applier_project'  => $this->getProject($judge->project_id),
                        'password'         => $pass
                    );
                    ...
                    return redirect()->route('Judge',$request->input('projectId'))
